...
public class ReelController : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < 12; i++)
        {
            Vector3 pos = new Vector3(0.0f, -100.0f + (100.0f * i), 0.0f);
    ...
    void Update()
    {
        ...
        {
            if (pos.localPosition.y % 100.0f < -5.0f)
            {  //絵柄をマスで固定するために回転スピードを弱める
                flg = 0;
                pos.localPosition = new Vector3(pos.localPosition.x, pos.localPosition.y - 2.0f, pos.localPosition.z);
            }
            else
            {    //固定完了
                if (flg == 0)
                { //トリガー
                    flg = 1;
                    int under = -1 * (int)(pos.localPosition.y / 100.0);  //何マス回転（移動）したか
